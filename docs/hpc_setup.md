...
`hpc_jobs/run_subJob.py` does not write these files directly to the sub-job's output
directory. Virtual Ecosystem writes its outputs (including the intermediate Zarr store)
to a staging directory under the compute node's local `$TMPDIR`. Once the run
completes, the Zarr store is deleted, and the remaining files (the NetCDF conversion,
compiled configuration, and logs) are moved into the sub-job's `array_subJob_<index>`
directory. This keeps the large number of small files generated by the Zarr store off
shared/quota-limited storage entirely.
...
Each array sub-job now produces a small, fixed set of output files (`pbs.log`,
