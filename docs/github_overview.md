...
1. Find the repository that you want to work on. Click on the green "code" button and
...
1. Decide where you want the repo to be stored on your local computer. Open a terminal
   window and navigate to this location (`cd FOLDERNAME` moves you into a folder, and
   `cd ..` moves you backwards.)
1. Once you have found the folder, paste `git clone SSH/HTTPS REPO URL` into the
   terminal. This should download a copy of the up-to-date code into your local
   computer.
1. Open VS Code (or another processor such as PyCharm) and open the folder that you just
   downloaded.
...
1. Pull the most recent version of main. Before you create a new branch, make sure your
   ...
   terminal.
1. Create a new branch. You can create a new branch in VS Code by clicking on the branch
...
1. First, check exactly what changes git has tracked. This ensures that you don't commit
   ...
   committed.
1. Check over your changes and stage them. Click on the files under changes and look
...
1. Once you are ready to commit all your staged changes, press the blue commit button.
...
1. Push your changes to github. To send your changes to github, press the "publish"
...
commands are `git status` to see what branch you're on and what files have changed; `git diff` to see what your changes are; `git add FILENAME` to stage changes in that file;
...
1. Add a description of what the PR accomplishes and request review from the relevant
   team members.

1. Wait for the reviews to come in: it makes life hard for reviewers if there are more
   changes coming into a PR while they are trying to review it!

1. Respond to reviews. Your reviewers can either give approval or request changes. Even
...
1. Once you have approvals from all reviewers, you can merge from the PR page, which
