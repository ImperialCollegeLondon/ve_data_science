...
## name: ruff-recursive-fix description: Run Ruff checks with optional scope and rule overrides, apply safe and unsafe autofixes iteratively, review each change, and resolve remaining findings with targeted edits or user decisions.
...
1. Else if `uv` is available and Ruff is managed through `uv`, use `uv run ruff`.
1. Else if `ruff` is available on `PATH`, use `ruff`.
1. Else if Python is available and Ruff is installed in that environment, use `python -m ruff`.
1. Else use any project-specific equivalent that invokes installed Ruff (for example `pipx run ruff`), or stop and ask the user.
...
1. Classify findings by type:
   ...
   - Autofixable unsafe.
   - Not autofixable.
1. If no findings remain, stop.

### 2. Safe Autofix Pass

1. Run Ruff with `--fix` using the same scope/options.
1. Review resulting diff carefully for semantic correctness and style consistency.
1. Run `<ruff_cmd> format` on the same scope.
1. Re-run `<ruff_cmd> check` to refresh remaining findings.
...
1. Review resulting diff carefully, prioritizing behavior-sensitive edits.
1. Run `<ruff_cmd> format` on the same scope.
1. Re-run `<ruff_cmd> check`.
...
1. Present valid options and trade-offs.
