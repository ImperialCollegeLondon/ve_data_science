...
#|     It runs all individual validation data library scripts sequentially and
#|     generates the plant validation data files for output comparison.
...
#|     metadata from all scripts in the validation data library workflow.
#|
...
#|
#| usage_notes: |
#|   Run this script to regenerate all plant validation data files.
#|   To update a single file, run the individual script directly.
#|   When adding a new script to the validation data library, add it to the
#|   scripts section above and to the scripts vector below in the correct order.
#| ---
    ...
    for (f in meta$output_files) {
      ...
      if (!is.null(f$variables)) {
        ...
        for (v in f$variables) {
          ...
          if (!is.null(v$references)) {
            summary_lines <- c(summary_lines, "           references:")
            for (ref in v$references) {
              if (!is.null(ref$citation)) {
                summary_lines <- c(
                  summary_lines,
                  sprintf("             - citation: %s", trimws(ref$citation))
                )
              }
              if (!is.null(ref$doi)) {
                summary_lines <- c(
                  summary_lines,
                  sprintf("               doi: %s", trimws(ref$doi))
                )
              }
              if (!is.null(ref$url)) {
                summary_lines <- c(
                  summary_lines,
                  sprintf("               url: %s", trimws(ref$url))
                )
              }
            }
          }
...
build_metadata_summary <- function(script_paths) {
  ...
  list(
    ...
    generated_by = "analysis/plant/output_data/validation/data_library/master_data_library.R",
