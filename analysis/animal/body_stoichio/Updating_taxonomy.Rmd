...
usage_notes: |
    Make sure input files are in the correct directories when globus data on ve_data_science repo is set up.
...
knitr::opts_knit$set(root.dir = "../../../data/primary/animal/body_stoichiometry")
# Make sure input files are in the directory when globus is set up
getwd()
```
...
na_counts_dat <- na_counts_dat[apply(na_counts_dat[, -1], 1, function(row_values) {
  !all(row_values == 0)
}), ]
...
na_counts_dat <- na_counts_dat[
  apply(na_counts_dat[, -1], 1, function(row_values) !all(row_values == 0)),
]
