...
#       time_index    → sequential time steps [e.g., 0, 1,2,......N]
#
#   Coordinates:
...
# Extract UTM coordinates from TOML file
longitude_UTM50N = cell_x.astype(np.float32)  # Easting (m)
latitude_UTM50N = cell_y.astype(np.float32)  # Northing (m)

# The original grid coordinates (cell_x, cell_y) represent projected UTM
# cell-centre positions. To ensure consistency with VE inputs, these
...
# Once we confirmed that our dataset is complete and our calculations
# are correct, we save it as a new NetCDF file.
